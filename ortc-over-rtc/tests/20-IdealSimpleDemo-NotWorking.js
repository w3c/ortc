...
An ideal simple demo that currently does not work.

**NEED:** Media engine must notify us of new `ssrc`s coming in without having registered
these first (registration is currently done via `<PeerConnection>.setRemoteDescription()`).
...
define([
    ...
    suite("IdealSimpleDemo-NotWorking", function() {
        ...
        test("run", function(done) {
            ...
            bob_connection.setDescription(JSON.parse(JSON.stringify(alice_connection.getDescription())), "remote");
            alice_connection.setDescription(JSON.parse(JSON.stringify(bob_connection.getDescription())), "remote");
